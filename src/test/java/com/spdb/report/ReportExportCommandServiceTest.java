...
class ReportExportCommandServiceTest {
    ...
    void returnsOnlyExportSummaryRowsForTheRequestedBatch() {
        String batchId = service.createAndStart();
        jdbc.update("""
                insert into ana_report_export_summary(batch_id, report_date, module_name, sent_transaction_count,
                ...
                values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, batchId, "20260726", "模块A", 11L, 4L, "0.70000000", 7L, 8L, 15L, 9L);
    ...
    @Test
    void returnsTrackingDetailsWithIssueHistorySnapshotForTheRequestedBatchInRowOrder() {
        ...
        jdbc.update("""
                insert into ana_tran_diff_tracking_export(source_batch_id, row_no, service_code, orig_error_code,
                    dest_error_code, tran_code, module_name, issue_id, historical_occurrence_count, first_seen_date, previous_seen_date)
                values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, "RPT1", 2L, "svc-b", "O2", "D2", "T2", "支付", 101L, 3L,
                LocalDate.parse("2026-07-01"), LocalDate.parse("2026-07-20"));
        jdbc.update("insert into ana_tran_diff_tracking_export(source_batch_id, row_no, service_code, tran_code) values (?, ?, ?, ?)",
                "RPT1", 1L, "svc-a", "T1");
        jdbc.update("""
                insert into ana_field_diff_tracking_export(source_batch_id, row_no, service_code, soap_field_name,
                    field_name, mapping_status, issue_id, historical_occurrence_count, first_seen_date, previous_seen_date)
                values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, "RPT1", 1L, "svc-a", "items.0", "items.0", "UNMAPPED", 102L, 2L,
        ...
        assertThat(service.findTransactionDetails("RPT1").get(1)).satisfies(row -> {
            ...
            assertThat(row.historicalOccurrenceCount()).isEqualTo(3L);
            assertThat(row.firstSeenDate()).isEqualTo(LocalDate.parse("2026-07-01"));
            assertThat(row.previousSeenDate()).isEqualTo(LocalDate.parse("2026-07-20"));
        });
        assertThat(service.findFieldDetails("RPT1")).singleElement().satisfies(row -> {
            ...
            assertThat(row.issueId()).isEqualTo(102L);
            assertThat(row.historicalOccurrenceCount()).isEqualTo(2L);
            assertThat(row.firstSeenDate()).isEqualTo(LocalDate.parse("2026-07-02"));
            assertThat(row.previousSeenDate()).isEqualTo(LocalDate.parse("2026-07-18"));
        });
    }
