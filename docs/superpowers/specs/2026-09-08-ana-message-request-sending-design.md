...
新增发送字段：`send_status`（PENDING/SENDING/SUCCESS/FAILED）、`send_time`、`send_http_status`、`send_error`、`send_attempts`。增加按状态和时间查询的索引。`system_config` 初始化 `micServId=10530013`，`authContent` 保留可配置项。
...
HTTP body 直接使用数据库中的原始二进制 `request_message`。Header 为 `micServId` 和 `authContent`；`micServId` 从 `system_config` 读取，`authContent` 当前计算器返回空字符串。按报文类型设置 JSON/XML 内容类型，未知类型使用 `application/octet-stream`。请求和响应均以原始二进制保存；页面展示时，`sop` 与 `sop2cbsp` 使用 HEX 字符串，`json` 与 `bzjson` 使用 JSON，`soap` 使用 XML。
